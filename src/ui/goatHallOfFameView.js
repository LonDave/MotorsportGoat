...
export class GoatHallOfFameView {
  static render(container, onNavigate) {
    ...
    // Calcolo dinamico in tempo reale della somma delle categorie
                ...
                <tbody>
                  ${hallOfFame.ranking.map((leg, idx) => `
                    <tr class="${leg.isPlayer ? 'player-hall-row' : ''}">
                      ...
                      <td>🏆 ${leg.titles}</td>
                      <td>🥇 ${leg.wins}</td>
                      <td class="points-cell text-right"><strong class="${leg.isPlayer ? 'gold-text' : ''}">${leg.goatScore}</strong></td>
                    </tr>
                  `).join('')}
                </tbody>
              </table>
